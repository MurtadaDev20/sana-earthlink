...
<?php
if (isset($_GET['rejection'])&& isset($_GET['invo_id']) && !empty($_GET['rejection'])) {
    $count = 0;
    global $con;
    $rejection = $_GET['rejection'];
    $invo_id = $_GET['invo_id'];
    $query = mysqli_query($con, "SELECT * FROM response where invo_id = '$rejection'");
...
<?php
        ...
        $query_delete_invoice = mysqli_query($con, "DELETE from response where driver_id ='$_SESSION[id_driver]' and invo_drive_id ='$invo_id'");

    $query_delete_invoice = mysqli_query($con, "UPDATE `invoice` SET `statues`='3' where id_invo ='$rejection'");
    $query_delete_request = mysqli_query($con, "UPDATE `request` SET `statues`='3' where invo_id ='$rejection'");
