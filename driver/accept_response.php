...
<?php
if (isset($_GET['accept']) && isset($_GET['invo_id']) && !empty($_GET['accept'])) {
    $count = 0;
    global $con;
    $accept = $_GET['accept'];
    $invo_id = $_GET['invo_id'];
    $query = mysqli_query($con, "SELECT * FROM response where invo_id = '$accept' ");


    $row_accept = mysqli_fetch_assoc($query);

    $received_date = $row_accept['date'];
    $coust_name = $row_accept['coust_id'];
...
?>
        ...
        // query Supervisor
        $query_supervisor = mysqli_query($con, "SELECT * FROM supervisor where id = '$coust_name'");
        $row_supervisor = mysqli_fetch_assoc($query_supervisor);
        $supervisor_name = $row_supervisor['fullName'];

        // query response
        $query_response = mysqli_query($con, "SELECT * FROM response where invo_id = '$accept'");
        ...
        ?>
                ...
                $query_delete_invoice = mysqli_query($con, "DELETE from response where driver_id ='$_SESSION[id_driver]' and invo_drive_id = '$accept'");
            ...
            <?php
                ...
                $query_delete_invoice = mysqli_query($con, "UPDATE `request` SET `statues`='2' where invo_id ='$accept' ");
                $query_delete_invoice = mysqli_query($con, "UPDATE `invoice` SET `statues`='2' where id_invo ='$accept' ");
                $query_delete_invoice_driver = mysqli_query($con, "DELETE FROM `invoice_driver` WHERE id_driv_invo ='$invo_id' ");
                $query_delete_res = mysqli_query($con, "DELETE FROM `response` WHERE invo_drive_id ='$invo_id' ");

                header("REFRESH:1;URL=response_manage.php");
            } else {
            ?>
                <script>
                    $(function() {
