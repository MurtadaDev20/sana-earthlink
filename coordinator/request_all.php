...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['com_req'])) {
    ...
    if (!empty($row['id'])) {
        ...
        $select_invo = mysqli_query($con , "SELECT * FROM invoice");
        
        

        $query = (mysqli_query($con, "SELECT * FROM cart c where c.coust_id = '$_SESSION[coust_id]' "));

        // $row_invo = mysqli_fetch_array($select_invo);
                    // $invoice_id = $row_invo['id'];

        while($row_invo = mysqli_fetch_array($select_invo))
        {
            $invoice_id = $row_invo['id_invo'];
        }
            while ($row_cart = mysqli_fetch_array($query)) 
            {
                ...
                $insert_req = mysqli_query($con, "INSERT INTO request ( `customer_name`, `coust_id`, `invo_id` , `category_id`, `product_id`,`quantity`, `source`, `req_from` , `req_to` , `other` , `coordinates`,`order_number`,`project_type`) 
                VALUES('$customer_name','$coust_id','$invoice_id','$category_id','$product_id','$quantity','$source','$from' , '$to' , '$other' , '$coordinates' , '$order_number' , '$project_type')");
